...
object AdminUtils extends Logging {
  ...
  def assignReplicasToBrokers(brokerMetadatas: Seq[BrokerMetadata],
    ...
    //System.out.println("【日志】AdminUtils.assignReplicasToBrokers() 分区副本到broker的分配计算入口")
  ...
  private def assignReplicasToBrokersRackUnaware(nPartitions: Int,
    ...
    for (_ <- 0 until nPartitions) {
      ...
      //System.out.println("【日志】AdminUtils.assignReplicasToBrokersRackUnaware() 分区副本到broker分配计算 currentPartitionId="+currentPartitionId+",replicaBuffer="+replicaBuffer)
      // 为下一个分区分配副本
  ...
  private def assignReplicasToBrokersRackAware(nPartitions: Int,
    ...
    //System.out.println("【日志】AdminUtils.assignReplicasToBrokersRackAware() 分配")
