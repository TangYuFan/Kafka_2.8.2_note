...
class Partition(val topicPartition: TopicPartition,
  ...
  def createLogIfNotExists(isNew: Boolean, isFutureReplica: Boolean, offsetCheckpoints: OffsetCheckpoints): Unit = {
    ...
    //System.out.println("【日志】Partition.createLogIfNotExists() 创建分区log文件")
  ...
  private[cluster] def createLog(isNew: Boolean, isFutureReplica: Boolean, offsetCheckpoints: OffsetCheckpoints): Log = {
    ...
    //System.out.println("【日志】Partition.createLog() 在当前broker节点初始化分区副本的本地目录 ")
  ...
  private def checkCurrentLeaderEpoch(remoteLeaderEpochOpt: Optional[Integer]): Errors = {
    if (!remoteLeaderEpochOpt.isPresent) {
      ...
      // 如果主副本本地 epoch 大于从副本请求的 epoch,说明 learder 已经切换,从副本在处理响应时会执行元数据更新
      if (localLeaderEpoch > remoteLeaderEpoch) {
        Errors.FENCED_LEADER_EPOCH
      }
        // 如果本地副本 epoch 小于从副本请求的 epoch 说明当前主副本落后,集群已经出现问题
  ...
  private def getLocalLog(currentLeaderEpoch: Optional[Integer], requireLeader: Boolean): Either[Log, Errors] = {
    checkCurrentLeaderEpoch(currentLeaderEpoch) match {

      // remoteLeaderEpoch = localLeaderEpoch
      case Errors.NONE =>
  ...
  def makeLeader(partitionState: LeaderAndIsrPartitionState,
    ...
    val (leaderHWIncremented, isNewLeader) = inWriteLock(leaderIsrUpdateLock) {
      ...
      //System.out.println("【日志】Partition.makeLeader() 当前分区:"+partitionState)
  ...
  def updateAssignmentAndIsr(assignment: Seq[Int],
    ...
    //System.out.println("【日志】Partition.updateAssignmentAndIsr() 保存分区分配信息和ISR状态")
  ...
  def checkEnoughReplicasReachOffset(requiredOffset: Long): (Boolean, Errors) = {
    ...
    //System.out.println("【日志】Partition.checkEnoughReplicasReachOffset() 检测指定offset之前的消息是否已经被ISR集合中足够多的follower副本确认 requiredOffset="+requiredOffset+",当前是否主副本:"+leaderLogIfLocal)
  ...
  private def maybeIncrementLeaderHW(leaderLog: Log, curTime: Long = time.milliseconds): Boolean = {
    ...
    //System.out.println("【日志】Partition.maybeIncrementLeaderHW() 尝试更新本地 hw 值");
  ...
  def maybeShrinkIsr(): Unit = {
    ...
    //System.out.println("【日志】Partition.maybeShrinkIsr() 检查ISR是否需要收缩");
  ...
  def getOutOfSyncReplicas(maxLagMs: Long): Set[Int] = {
    ...
    //System.out.println("【日志】Partition.getOutOfSyncReplicas() 获取lag落后副本 maxLagMs="+maxLagMs);
  ...
  private def doAppendRecordsToFollowerOrFutureReplica(records: MemoryRecords, isFuture: Boolean): Option[LogAppendInfo] = {
    ...
    //System.out.println("【日志】Partition.doAppendRecordsToFollowerOrFutureReplica()")
...
//System.out.println("【日志】Partition.appendRecordsToFollowerOrFutureReplica()");
  ...
  def appendRecordsToLeader(records: MemoryRecords, origin: AppendOrigin, requiredAcks: Int): LogAppendInfo = {
    ...
    //System.out.println("【日志】Partition.appendRecordsToLeader()");
...
//System.out.println("【日志】Partition.readRecords() 读取当前主副本中未同步记录 fetchOffset="+fetchOffset);
  ...
  def lastOffsetForLeaderEpoch(currentLeaderEpoch: Optional[Integer],
    ...
    //System.out.println("【日志】Partition.lastOffsetForLeaderEpoch() currentLeaderEpoch="+currentLeaderEpoch+",leaderEpoch="+leaderEpoch+",fetchOnlyFromLeader="+fetchOnlyFromLeader);

    // 返回小于或等于requestedLeaderEpoch的最大Epoch的LEO。
    // LEO被定义为大于requestedLeaderEpoch的第一个Epoch的startOffset；
    // 或者定义为latestLeaderEpoch的LEO，如果requestedLeaderEpoch等于latestLeaderEpoch。
    ...
    inReadLock(leaderIsrUpdateLock) {
      ...
      //System.out.println("【日志】Partition.lastOffsetForLeaderEpoch() fetchOnlyFromLeader="+fetchOnlyFromLeader+",localLogOrError="+localLogOrError);

      localLogOrError match {
        // 前面返回 Left(partitionLog)
        case Left(localLog) =>
          //
