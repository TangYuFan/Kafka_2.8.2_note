...
class QueuedEvent(val event: ControllerEvent,
  ...
  def process(processor: ControllerEventProcessor): Unit = {
    //System.out.println("【日志】ControllerEventManager.process() 运行事件:"+event)
...
class ControllerEventManager(controllerId: Int,
  ...
  def put(event: ControllerEvent): QueuedEvent = inLock(putLock) {
    ...
    //System.out.println("【日志】ControllerEventManager.put() 向事件队列queue中添加事件:"+event);
  ...
  class ControllerEventThread(name: String) extends ShutdownableThread(name = name, isInterruptible = false) {
    ...
    override def doWork(): Unit = {
      ...
      //System.out.println("【日志】ControllerEventManager.doWork() 从事件队列queue中获取到事件:"+dequeued)
