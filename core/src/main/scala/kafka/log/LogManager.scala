...
class LogManager(logDirs: Seq[File],
  ...
  def checkpointLogStartOffsets(): Unit = {
    ...
    //System.out.println("【日志】LogManager.checkpointLogStartOffsets()");
  ...
  private def checkpointLogStartOffsetsInDir(logDir: File, logsToCheckpoint: Map[TopicPartition, Log]): Unit = {
    ...
    //System.out.println("【日志】LogManager.checkpointLogStartOffsetsInDir() logDir="+logDir+",logsToCheckpoint="+logsToCheckpoint);
  ...
  def getOrCreateLog(topicPartition: TopicPartition, isNew: Boolean = false, isFuture: Boolean = false): Log = {
    ...
    //System.out.println("【日志】LogManager.getOrCreateLog() 当前主题分区:"+topicPartition)
    ...
    logCreationOrDeletionLock synchronized {
      getLog(topicPartition, isFuture).getOrElse {
        ...
        // 拼接log目录,调用 createLogDirectory 创建
        val logDir = logDirs
...
//        info(s"Created log for partition $topicPartition in $logDir with properties ${config.overriddenConfigsAsLoggableString}")
  ...
  private[log] def createLogDirectory(logDir: File, logDirName: String): Try[File] = {
    ...
    //System.out.println("【日志】LogManager.createLogDirectory() 创建日志目录:"+logDir)
