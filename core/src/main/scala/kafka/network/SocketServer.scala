...
class SocketServer(val config: KafkaConfig,
  ...
  def startup(startProcessingRequests: Boolean = true,
    ...
    //System.out.println("【日志】SockerServer.startup() 网络启动")

    this.synchronized {
      // 启动 data、control 的 Acceptor、Processor
  ...
  def startProcessingRequests(authorizerFutures: Map[Endpoint, CompletableFuture[Void]] = Map.empty): Unit = {
    ...
    //System.out.println("【日志】SocketServer.startProcessingRequests()")
  ...
  private def startAcceptorAndProcessors(threadPrefix: String,
    ...
    //System.out.println("【日志】SocketServer.startAcceptorAndProcessors() threadPrefix="+threadPrefix)
  ...
  private def startDataPlaneProcessorsAndAcceptors(authorizerFutures: Map[Endpoint, CompletableFuture[Void]]): Unit = {
    //System.out.println("【日志】SocketServer.startDataPlaneProcessorsAndAcceptors()")
  ...
  private def startControlPlaneProcessorAndAcceptor(authorizerFutures: Map[Endpoint, CompletableFuture[Void]]): Unit = {
    //System.out.println("【日志】SocketServer.startControlPlaneProcessorAndAcceptor()")
  ...
  private def createDataPlaneAcceptorsAndProcessors(dataProcessorsPerListener: Int,
    ...
    //System.out.println("【日志】SocketServer.createDataPlaneAcceptorsAndProcessors() 创建 Acceptor/Processor")
  ...
  private def createControlPlaneAcceptorAndProcessor(endpointOpt: Option[EndPoint]): Unit = {
    ...
    //System.out.println("【日志】SockerServer.createControlPlaneAcceptorAndProcessor()")
  ...
  private def createAcceptor(endPoint: EndPoint, metricPrefix: String) : Acceptor = {
    ...
    //System.out.println("【日志】SocketServer.createAcceptor() 为每个EndPoint创建客户端连接处理器Acceptor,endPoint="+endPoint)
  ...
  private def addDataPlaneProcessors(acceptor: Acceptor, endpoint: EndPoint, newProcessorsPerListener: Int): Unit = {
    ...
    //System.out.println("【日志】SocketServer.addDataPlaneProcessors() newProcessorsPerListener="+newProcessorsPerListener)
  ...
  protected[network] def newProcessor(id: Int, requestChannel: RequestChannel, connectionQuotas: ConnectionQuotas, listenerName: ListenerName,
    ...
    //System.out.println("【日志】SocketServer.newProcessor() id="+id+",requestChannel="+requestChannel)
...
private[kafka] class Acceptor(val endPoint: EndPoint,
   ...
   */
  def run(): Unit = {
    //System.out.println("【日志】SocketServer.run()")
    serverChannel.register(nioSelector, SelectionKey.OP_ACCEPT)
    startupComplete()
    try {
      //System.out.println("【日志】SocketServer.acceptNewConnections() 死while循环监听客户端链接,注册 SelectionKey.OP_ACCEPT 事件")
      while (isRunning) {
        try {
  ...
  private def openServerSocket(host: String, port: Int): ServerSocketChannel = {
    ...
    //System.out.println("【日志】SocketServer.openServerSocket()")
  ...
  private def assignNewConnection(socketChannel: SocketChannel, processor: Processor, mayBlock: Boolean): Boolean = {
    ...
    //System.out.println("【日志】SocketServer.assignNewConnection() 监听到新的连接")
...
private[kafka] class Processor(val id: Int,
  ...
  override def run(): Unit = {
    ...
    //System.out.println("【日志】SocketServer.run() 开始运行Server主任务")
  ...
  protected[network] def sendResponse(response: RequestChannel.Response, responseSend: Send): Unit = {
    ...
    //System.out.println("【日志】SocketServer.sendResponse() 响应客户端:"+response)
          ...
          case Some(channel) =>
            ...
            if (header.apiKey == ApiKeys.SASL_HANDSHAKE && channel.maybeBeginServerReauthentication(receive,
              ...
              if (channel.serverAuthenticationSessionExpired(nowNanos)) {
                ...
                //System.out.println("【日志】SocketServer.processCompletedReceives() 调用selector轮询到的客户端请求")
...
//System.out.println("【日志】SocketServer.processCompletedSends() 处理客户端请求的响应")
   ...
   */
  def accept(socketChannel: SocketChannel, mayBlock: Boolean, acceptorIdlePercentMeter: com.yammer.metrics.core.Meter): Boolean = {
    val accepted = {
  ...
  private def configureNewConnections(): Unit = {
    ...
    while (connectionsProcessed < connectionQueueSize && !newConnections.isEmpty) {
      ...
      try {
        //System.out.println("【日志】SocketServer.configureNewConnections() 从newConnections队列中获取到新的连接")
