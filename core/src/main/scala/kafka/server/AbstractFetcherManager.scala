...
abstract class AbstractFetcherManager[T <: AbstractFetcherThread](val name: String, clientId: String, numFetchers: Int)
  ...
  def addFetcherForPartitions(partitionAndOffsets: Map[TopicPartition, InitialFetchState]): Unit = {
    lock synchronized {
      ...
      //System.out.println("【日志】ReplicaFetcherThread.addFetcherForPartitions() 创建FETCH线程,定时向主副本发送FETCH请求");
  ...
  protected def addPartitionsToFetcherThread(fetcherThread: T, initialOffsetAndEpochs: collection.Map[TopicPartition, InitialFetchState]): Unit = {
