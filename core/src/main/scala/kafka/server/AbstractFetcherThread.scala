...
//System.out.println("【日志】AbstractFetcherThread.doWork()")
    maybeTruncate()
    maybeFetch()
  }
...
//System.out.println("【日志】AbstractFetcherThread.maybeFetch() 同副本复制处理入口");
  ...
  private def fetchTruncatingPartitions(): (Map[TopicPartition, EpochData], Set[TopicPartition]) = inLock(partitionMapLock) {
    ...
    //System.out.println("【日志】AbstractFetcherThread.fetchTruncatingPartitions() 将本地所有处于截断中状态的分区依据有无Leader Epoch值进行分组");
  ...
  private def truncateToEpochEndOffsets(latestEpochsForPartitions: Map[TopicPartition, EpochData]): Unit = {
    ...
    //System.out.println("【日志】AbstractFetcherThread.truncateToEpochEndOffsets() 有Leader Epoch值的分区，将日志截断到 leaderEpoch值对应的位移值处");
    ...
    inLock(partitionMapLock) {
      // 在想主副本发送阻塞请求过程中也可能有的副本发生过主副本切换,这里筛选出来
      val epochEndOffsets = endOffsets.filter { case (tp, _) =>
        ...
        val partitionEpochRequest = latestEpochsForPartitions.getOrElse(tp, {
          throw new IllegalStateException(s"Leader replied with partition $tp not requested in OffsetsForLeaderEpoch request")
        })
      ...
      // 调用maybeTruncateToEpochEndOffsets进行日志截断
      ...
      //partitionsWithError：出错的分区,下面就专门处理出错的分区
  ...
  private[server] def truncateToHighWatermark(partitions: Set[TopicPartition]): Unit = inLock(partitionMapLock) {
    ...
    //System.out.println("【日志】AbstractFetcherThread.truncateToHighWatermark() 没有Leader Epoch值的分区，将日志截断到hw处");
  ...
  private def maybeTruncateToEpochEndOffsets(fetchedEpochs: Map[TopicPartition, EpochEndOffset],
    ...
    //System.out.println("【日志】AbstractFetcherThread.maybeTruncateToEpochEndOffsets() 根据返回的 leaderEpoch 进行截断:fetchedEpochs="+fetchedEpochs);

    val fetchOffsets = mutable.HashMap.empty[TopicPartition, OffsetTruncationState]
    val partitionsWithError = mutable.HashSet.empty[TopicPartition]

    // Map[TopicPartition, EpochEndOffset] 遍历每个分区以及 epoch 值
...
//System.out.println("【日志】AbstractFetcherThread.processFetchRequest() 发送FETCH请求");
