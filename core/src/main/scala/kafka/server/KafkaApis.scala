 ...
 */
class KafkaApis(val requestChannel: RequestChannel, // 请求channel用于数据响应
  ...
  private def maybeForwardToController(
    ...
    //System.out.println("【日志】KafkaApis.maybeForwardToController() 请求转发给控制节点")
  ...
  override def handle(request: RequestChannel.Request): Unit = {
    ...
    try {
      ...
      //System.out.println("【日志】KafkaApis.handle() 处理客户端请求:"+request.header.apiKey)
      ...
      request.header.apiKey match {
        case ApiKeys.PRODUCE => handleProduceRequest(request) // 生产者消息投递
        case ApiKeys.FETCH => handleFetchRequest(request)   // 消费者抓取消息
  ...
  def handleLeaderAndIsrRequest(request: RequestChannel.Request): Unit = {
    ...
    //System.out.println("【日志】KafkaApis.handleLeaderAndIsrRequest() 处理ISR列表请求")
    ...
    if (isBrokerEpochStale(zkSupport, leaderAndIsrRequest.brokerEpoch)) {
      ...
      // 处理副本
      val response = replicaManager.becomeLeaderOrFollower(correlationId, leaderAndIsrRequest, RequestHandlerHelper.onLeadershipChange(groupCoordinator, txnCoordinator, _, _))
  ...
  def handleUpdateMetadataRequest(request: RequestChannel.Request): Unit = {
    ...
    //System.out.println("【日志】KafkaApis.handleUpdateMetadataRequest() 更新元数据:"+request)
    ...
    // 这里主要是根据 leader epoch 机制判断
    // 比较当前节点缓存的 controllerEpoch 和 发起元数据更新请求的 controllerEpoch 谁大
    // 如果收到了更小 epoch 任期的 Controller 节点的元数据更新请求就不处理因为他现在已经不是集群内真正的 controller 了
    if (isBrokerEpochStale(zkSupport, updateMetadataRequest.brokerEpoch)) {
      ...
      requestHelper.sendResponseExemptThrottle(request, new UpdateMetadataResponse(new UpdateMetadataResponseData().setErrorCode(Errors.STALE_BROKER_EPOCH.code)))
    }
    else {
      // 更新元数据之后,有待删除的分区则进一步处理
  ...
  def handleProduceRequest(request: RequestChannel.Request): Unit = {
    ...
    //System.out.println("【日志】KafkaApis.handleProduceRequest() 处理生产者 PRODUCE 提交请求");
...
//System.out.println("【日志】KafkaApis.handleFetchRequest() 处理从副本FETCH同步请求,请求节点 fetchRequest.isFromFollower="+fetchRequest.isFromFollower);
  ...
  def handleTopicMetadataRequest(request: RequestChannel.Request): Unit = {
    ...
    //System.out.println("【日志】KafkaApis.handleTopicMetadataRequest() request="+request)
    ...
    //System.out.println("【日志】KafkaApis.handleTopicMetadataRequest() 当前所有活跃broker="+brokers)
    //System.out.println("【日志】KafkaApis.handleTopicMetadataRequest() 当前所有集群元数据="+completeTopicMetadata)
  ...
  def handleCreateTopicsRequest(request: RequestChannel.Request): Unit = {
    ...
    //System.out.println("【日志】KafkaApis.handleCreateTopicsRequest() 当前节点是否控制节点："+(zkSupport.controller.isActive))
    ...
    if (!zkSupport.controller.isActive) {
      ...
      // 调用 ZkAdminManager 执行主题创建,也就是将主题和分区信息记录到zk上
  ...
  def handleCreatePartitionsRequest(request: RequestChannel.Request): Unit = {
    ...
    //System.out.println("【日志】KafkaApis.handleCreatePartitionsRequest() 处理新增分区的请求");
  ...
  def handleOffsetForLeaderEpochRequest(request: RequestChannel.Request): Unit = {
    ...
    //System.out.println("【日志】KafkaApis.handleOffsetForLeaderEpochRequest() 处理epoch获取请求");
  ...
  private def isBrokerEpochStale(zkSupport: ZkSupport, brokerEpochInRequest: Long): Boolean = {
    ...
    //System.out.println("【日志】Kafkaapis.isBrokerEpochStale() 校验epoch,controllerEpoch1="+zkSupport.controller.brokerEpoch+",controllerEpoch2="+brokerEpochInRequest);
