...
class KafkaRequestHandler(id: Int,
  ...
  //System.out.println("【日志】KafkaRequestHandler.new()")
  this.logIdent = "[Kafka Request Handler " + id + " on Broker " + brokerId + "], "
  private val shutdownComplete = new CountDownLatch(1)
  @volatile private var stopped = false

  def run(): Unit = {
    //System.out.println("【日志】KafkaRequestHandler.run() 调用[requestChannel]从[requestQueue]中轮询客户端请求")
    while (!stopped) {
      ...
      if(req!=null){
        //System.out.println("【日志】KafkaRequestHandler.run() 从[requestQueue]中获取的请求:"+req.getClass)
      }
...
class KafkaRequestHandlerPool(val brokerId: Int,
  ...
  //System.out.println("【日志】KafkaRequestHandlerPool.new() 创建请求处理池 brokerId="+brokerId+",numThreads="+numThreads)
  ...
  def createHandler(id: Int): Unit = synchronized {
    ...
    //System.out.println("【日志】KafkaRequestHandler.createHandler() id="+id)
