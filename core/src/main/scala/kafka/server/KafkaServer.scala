...
class KafkaServer(
  ...
  override def startup(): Unit = {
    ...
    //System.out.println("【日志】KafkaServer.startup() 服务端启动")
  ...
  private def initZkClient(time: Time): Unit = {
    ...
    def createZkClient(zkConnect: String, isSecure: Boolean) = {
      KafkaZkClient(zkConnect, isSecure, config.zkSessionTimeoutMs, config.zkConnectionTimeoutMs, config.zkMaxInFlightRequests, time, name = Some("Kafka server"), zkClientConfig = Some(zkClientConfig))
    }
