...
class ZkMetadataCache(brokerId: Int) extends MetadataCache with Logging {
  ...
  private def addOrUpdatePartitionInfo(partitionStates: mutable.AnyRefMap[String, mutable.LongMap[UpdateMetadataPartitionState]],
    ...
    //System.out.println("【日志】MetaCache.addOrUpdatePartitionInfo topic="+topic+",partitionId="+partitionId+",stateInfo="+stateInfo);
  ...
  def updateMetadata(correlationId: Int, updateMetadataRequest: UpdateMetadataRequest): Seq[TopicPartition] = {
    ...
    //System.out.println("【日志】MetadataCache.updateMetadata() 待更新的集群元数据:"+updateMetadataRequest);
    ...
    inWriteLock(partitionMetadataLock) {
      ...
      if (!updateMetadataRequest.partitionStates.iterator.hasNext) {
        ...
        //System.out.println("【日志】MetaCache.updateMetadata() 元数据更新完成="+metadataSnapshot);
