...
class ReplicaFetcherManager(brokerConfig: KafkaConfig,
  ...
  override def createFetcherThread(fetcherId: Int, sourceBroker: BrokerEndPoint): ReplicaFetcherThread = {
    ...
    //System.out.println("【日志】ReplicaFetcherManager.createFetcherThread() 创建FETCH线程");

    val prefix = threadNamePrefix.map(tp => s"$tp:").getOrElse("")
    val threadName = s"${prefix}ReplicaFetcherThread-$fetcherId-${sourceBroker.id}"
    new ReplicaFetcherThread(threadName, fetcherId, sourceBroker, brokerConfig, failedPartitions, replicaManager, metrics, time, quotaManager)
  }
