...
class RequestHandlerHelper(requestChannel: RequestChannel,
  ...
  def sendResponseMaybeThrottleWithControllerQuota(controllerMutationQuota: ControllerMutationQuota,
    ...
    //System.out.println("【日志】RequestHandlerHelper.sendResponseMaybeThrottleWithControllerQuota() 创建主题完成,调用sendResponse()进行响应")
  ...
  def sendResponse(request: RequestChannel.Request,
    ...
    //System.out.println("【日志】RequestHandlerHelper.sendResponse() 响应 reqApiKey="+request.header.apiKey())

    val response = responseOpt match {
      case Some(response) =>
        //System.out.println("【日志】RequestHandlerHelper.sendResponse() 响应 response="+response)
