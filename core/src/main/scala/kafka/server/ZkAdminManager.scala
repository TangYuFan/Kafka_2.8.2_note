...
class ZkAdminManager(val config: KafkaConfig,
  ...
  private def validateTopicCreatePolicy(topic: CreatableTopic,
    ...
    //System.out.println("【日志】ZkAdminManager.validateTopicCreatePolicy() 验证分配计算结果 assignments="+assignments)
  ...
  private def maybePopulateMetadataAndConfigs(metadataAndConfigs: Map[String, CreatableTopicResult],
    ...
    //System.out.println("【日志】ZkAdminManager.maybePopulateMetadataAndConfigs()")
  ...
  def createTopics(timeout: Int,
    ...
    //System.out.println("【日志】ZkAdminManager.createTopics() 待创建主题集合:"+toCreate)

    // 1. map over topics creating assignment and calling zookeeper
    val brokers = metadataCache.getAliveBrokers.map { b => kafka.admin.BrokerMetadata(b.id, Option(b.rack)) }

    // 获取到活跃 brokers
    //System.out.println("【日志】ZkAdminManager.createTopics() 当前活跃broker:"+brokers)

    // 遍历每个需要创建的 topic 执行下面过程
  ...
  def createPartitions(timeout: Int,
    ...
    //System.out.println("【日志】ZkAdminManager.createPartitions() 创建分区")
