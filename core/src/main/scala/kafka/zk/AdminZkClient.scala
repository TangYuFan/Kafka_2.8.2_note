...
class AdminZkClient(zkClient: KafkaZkClient) extends Logging {
  ...
  def createTopicWithAssignment(topic: String,
    ...
    //System.out.println("【日志】AdminZkClient.createTopicWithAssignment() 将topic和分区分配信息写入zk入口: topic="+topic+",partitionReplicaAssignment="+partitionReplicaAssignment)
...
//      s"assignment $partitionReplicaAssignment")
  ...
  def validateTopicCreate(topic: String,
    ...
    //System.out.println("【日志】AdminZkClient.validateTopicCreate() 校验zk上是否有相同的topic,以及副本分区是否重复等等")
  ...
  def createPartitionsWithAssignment(topic: String,
    ...
    //System.out.println("【日志】AdminZkClient.createPartitionsWithAssignment() 创建分区partition入口")
