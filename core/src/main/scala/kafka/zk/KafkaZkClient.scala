...
class KafkaZkClient private[zk] (zooKeeperClient: ZooKeeperClient, isSecure: Boolean, time: Time) extends AutoCloseable with
  ...
  private[kafka] def createSequentialPersistentPath(path: String, data: Array[Byte]): String = {
    ...
    //System.out.println("【日志】KafkaZkClient.createSequentialPersistentPath() 写入zk数据path="+path)
  ...
  def setOrCreateEntityConfigs(rootEntityType: String, sanitizedEntityName: String, config: Properties) = {
    ...
    //System.out.println("【日志】KafkaZkClient.setOrCreateEntityConfigs() 在zk上创建路径")
  ...
  def setTopicAssignment(topic: String,
    ...
    //System.out.println("【日志】KafkaZkClient.setTopicAssignment() 写入分区信息到zk")
  ...
  def createTopicAssignment(topic: String, topicId: Option[Uuid], assignment: Map[TopicPartition, Seq[Int]]): Unit = {
    ...
    //System.out.println("【日志】KafkaZkClient.createTopicAssignment() 写入分区信息到zk")
  ...
  // 发布ISR变更的信息到zk上,由Controller进行监听
  def propagateIsrChanges(isrChangeSet: collection.Set[TopicPartition]): Unit = {
    //System.out.println("【日志】KafkaZkClient.propagateIsrChanges() 发布ISR变更信息 "+isrChangeSet)
    val isrChangeNotificationPath: String = createSequentialPersistentPath(IsrChangeNotificationSequenceZNode.path(), IsrChangeNotificationSequenceZNode.encode(isrChangeSet))
  ...
  private[kafka] def createRecursive(path: String, data: Array[Byte] = null, throwIfPathExists: Boolean = true) = {
    ...
    //System.out.println("【日志】KafkaZkClient.createRecursive() 数据写入路径 path="+path)
