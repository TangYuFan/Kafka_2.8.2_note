...
public final class QuorumController implements Controller {
    ...
    private <T> CompletableFuture<T> appendWriteEvent(String name, ControllerWriteOperation<T> op) {
        ...
        //System.out.println("【日志】QuorumController.appendWriteEvent() name="+name);
    ...
    private void replay(ApiMessage message, long offset) {
        ...
        //System.out.println("【日志】QuorumController.replay() 消息类型:"+MetadataRecordType.fromId(message.apiKey()));
    ...
    public CompletableFuture<CreateTopicsResponseData> createTopics(CreateTopicsRequestData request) {
        ...
        //System.out.println("【日志】QuorumController.createTopics() 创建主题");

        if (request.topics().isEmpty()) {
            return CompletableFuture.completedFuture(new CreateTopicsResponseData());
        }
        // 缓存一个 replicationControl.createTopics 事件到 KafkaEventQueue 中
        // 后续由 KafkaEventQueue 进行执行
        return appendWriteEvent("createTopics", () -> replicationControl.createTopics(request));
    }
